...
<p class="text-muted text-center mail-text">Sistema de inventario y facturación</p>
</div>
...
//Consulta para verificar si es el administrador
  $quer = $conn->prepare("SELECT * FROM usuario  where usuario = '$userr' and clave = MD5('$passwr')");
     $quer->execute();
...
if ($rowcount) {
  //conexion a db
	$con = mysqli_connect("localhost","root","","db_barrera");
...
$date = date('d-m-Y'); 
$filename = $folder."barreraBD_".$date; 
