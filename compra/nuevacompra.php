...
if (!empty($_GET['compraprint'])) {

  $print = $_GET['compraprint'];

include_once('../conexion/conexion.php');
    $conn = conexion();

 
      $consultar = $conn->prepare("SELECT  max(idcompra) as imprime FROM compra");
       $consultar->execute();

    $dat = $consultar->fetch(PDO::FETCH_ASSOC);
    $cod = $dat['imprime'];

    if ($print == "4e8fe0ea1b43e476f0ed0655668f3a7fpos0") {

      ?>


<script src="../js/print.js"></script>
      <body target="_blank" onload="imprimircompra('<?php echo $cod; ?>');"></body>
...
<select name="cbo_producto" id="cbo_producto" style=" height: 300%;width: 77%" class="select form-control "  class="col-md-2 form-control">
          <option value="0">Seleccione un producto</option>
           <?php
                  ...
                  $sql = "SELECT * FROM producto where estado='Activo' order by idproducto desc";
