...
class Producto
{
	...
	function guardarDetallecompra($fechav,$cantidad, $precio, $subtotal,$idcompra,$idproducto){
		$sql = "INSERT INTO detallecompra (fechav,cantidad,precio,subtotal,idcompra,idproducto) values ('$fechav','$cantidad','$precio','$subtotal','$idcompra','$idproducto')";
		global $cnx;
		return $cnx->query($sql);
	}

	function actualizar($cantidad,$precio,$total,$fechav,$idproducto){

		$sql ="UPDATE producto SET cantidad= cantidad+'$cantidad',preciounitario = $precio,total = cantidad*'$precio', fechav ='$fechav'  WHERE idproducto = '$idproducto'";
