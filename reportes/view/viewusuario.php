...
//si se cumple a condicion
    ...
    //addslashes(str) para no permitir sqlinjection
		 ...
		 $aColumns = array('idusuario','usuario.nombre','usuario.apellido','usuario.email','usuario.estado','tipousuario.nombre');//Columnas de busqueda
...
$count_query = "SELECT usuario.*,tipousuario.nombre as nombu FROM $sTable INNER JOIN tipousuario on usuario.idtipousuarios=tipousuario.idtipousuario $sWhere ";
//Ejecuta el query
...
$sql = "SELECT usuario.*,tipousuario.nombre as nombu FROM $sTable INNER JOIN tipousuario on usuario.idtipousuarios=tipousuario.idtipousuario $sWhere ORDER BY idusuario desc LIMIT $offset,$per_page";
